...
DOCUMENTATION = '''
---
module: delete_tenant
short_description: Deletes a compute node and all volumes attach to that compute node.
description:
  - This module deletes a compute node and all volumes attach to that compute node from a Hitachi Virtual Storage Platform One SDS Block storage system.
options:
  management_address:
    description:
...
def hitachi_vssb_main():
    ...
    logger.info("Intialized delete_tenant task")
    ...
    try:
        ...
        response = executors.delete_tenant()
    ...
    logger.info("Completed delete_tenant task")
